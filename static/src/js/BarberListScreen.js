odoo.define('md_barber.BarberListScreen', function(require) {
    'use strict';
     ...
     */
    class BarberListScreen extends PosComponent {
        constructor() {
            super(...arguments);
            ...
            this.updateBarberList = debounce(this.updateBarberList, 70);
        }
        // Lifecycle hooks
        back() {
        ...
        get barbers() {
            ...
            return resData.sort(function (a, b) { return (a.name || '').localeCompare(b.name || '') });
        }
        ...
        get nextButton() {
            if (!this.props.client) {
                return { command: 'set', text: this.env._t('Set Barber') };
            } else if (this.props.client && this.props.client === this.state.selectedClient) {
                return { command: 'deselect', text: this.env._t('Deselect Barber') };
            } else {
                return { command: 'set', text: this.env._t('Change Barber') };
            }
        }
    ...
    BarberListScreen.template = 'BarberListScreen';

    Registries.Component.add(BarberListScreen);
